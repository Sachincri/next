...
import { DollarSign, ShoppingCart, Package, Users, TrendingUp, Star, Wifi, WifiOff, CheckCircle, MessageSquareText } from "lucide-react"
              ...
              <CardContent>
                ...
                {!loading && (
                    <div className="flex flex-col gap-1">
                        ...
                        {stat.title === "Total Revenue" && (data?.deductions?.totalRefunds ?? 0) > 0 && (
                            <p className="text-[10px] text-red-500 font-medium">
                                (- {formatCurrency(data?.deductions?.totalRefunds || 0)} Refunds)
                            </p>
                        )}
                    </div>
                )}
                ...
                <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-border/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {isZero ? (
                        <p className="text-sm font-medium text-muted-foreground/60 italic">No activity yet</p>
                      ) : (
                        <p className="text-sm font-bold">
                          {typeof item.value === 'number' && item.label.includes('Revenue') ? formatCurrency(item.value) : item.value}
                        </p>
                      )}
                    </div>
                  </div>
                  {!isZero && item.growth !== undefined && item.growth !== 0 && (
        ...
        {[
          {
            ...
            icon: Star,
            iconColor: safeGetKpi("customerSatisfaction").value < 2.0 && !loading
              ? "text-red-600 bg-red-100 dark:bg-red-900/20"
              : "text-amber-600 bg-amber-100 dark:bg-amber-900/20",
            cardClass: safeGetKpi("customerSatisfaction").value < 2.0 && !loading
          ...
          },
          {
            ...
            value: loading ? "..." : safeGetKpi("abandonedCarts").value.toLocaleString(),
            change: 0,
            icon: ShoppingCart,
            iconColor: "text-red-500 bg-red-100 dark:bg-red-900/20",
            action: safeGetKpi("abandonedCarts").value > 0 ? (
              <Button
                size="sm"
                variant="outline"
                className="mt-2 h-7 text-xs w-full border-red-200 text-red-600 hover:bg-red-100 hover:text-red-700 gap-1.5"
                onClick={() => toast.success(`Recovery reminder sent to ${safeGetKpi("abandonedCarts").value} user(s) via WhatsApp & Email!`)}
              >
          ...
          const Icon = stat.icon;
          return (
            <Card key={i} className={`hover:shadow-md transition-shadow ${(stat as any).cardClass || ""}`}>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium text-muted-foreground">{stat.title}</CardTitle>
                <div className={`p-2 rounded-full ${stat.iconColor}`}>
                  <Icon className="h-4 w-4" />
                  ...
                  <div key={product._id} className="flex items-center justify-between p-3 rounded-lg bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 group hover:border-red-200 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className={`text-xs font-medium ${product.stock < 10 ? 'text-orange-500 animate-pulse' : 'text-red-600'}`}>{product.stock} units remaining</p>
                      </div>
                    </div>
