...
export const ProductSlider: React.FC<ProductSliderProps> = ({ heading, products }) => {
  ...
  return (
    <div className="w-full bg-white md:px-4">
      {/* Heading */}
      {heading && (
        <div className="sm:px-6 py-4">
          <div className="flex items-center justify-between mb-4 px-2 sm:px-0">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800">{heading}</h2>
        ...
        <div className="flex sm:hidden overflow-x-auto snap-x snap-mandatory gap-4 pb-4 mx-2 no-scrollbar">
          {products.map(product => (
            <Link
              ...
              className="flex-shrink-0 w-[160px] snap-start bg-white transition-all duration-300 group"
            >
              <div className="relative overflow-hidden bg-gray-50 rounded-md aspect-square mb-2">
                <img
                  src={
              ...
              <h3 className="text-xs text-gray-800 mb-1 line-clamp-2 h-8">{product.name}</h3>
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold">₹{(product.sellingPrice)?.toLocaleString()}</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="hidden sm:block relative px-2 pb-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
            {products
              ...
              .map(product => (
                <Link key={product._id} href={`/product/${product._id}`} className="bg-white transition-all duration-300 group cursor-pointer">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-base font-medium">₹{(product.sellingPrice)?.toLocaleString()}</span>
          ...
          {canGoPrev && (
            <button
              ...
              className="z-20 absolute left-0 top-1/2 -translate-y-1/2 bg-white/90 backdrop-blur-sm w-8 sm:w-11 h-16 sm:h-24 flex items-center justify-center shadow-lg border rounded-r-md hover:bg-white transition-colors"
            >
          ...
          {canGoNext && (
            <button
              ...
              className="z-20 absolute right-0 top-1/2 -translate-y-1/2 bg-white/90 backdrop-blur-sm w-8 sm:w-11 h-16 sm:h-24 flex items-center justify-center shadow-lg border rounded-l-md hover:bg-white transition-colors"
            >
